...
@section('javascript')
    ...
    <script>
        ...
        $(document).ready(function() {
                            ...
                            <div style="display: flex; align-items: flex-start;">
                                <div style="flex-grow: 1; line-height: 1.4;">
                                    <div style="font-size: 15px; font-weight: bold; color: #2c3e50; margin-bottom: 5px;">
                                        <i class="fa-solid fa-pills" style="color: #3498db;"></i> ${product.item.name}
                                    </div>
            ...
            @if(isset($sale))
                @php
                    ...
                    $groupedDetailsPHP = $sale->saleDetails->groupBy('item_stock_id');
                    ...
                    foreach($groupedDetailsPHP as $itemStockId => $details) {
                        ...
                        if ($itemStock && $itemStock->dispensed === 'Fraccionado' && $itemStock->dispensedQuantity > 0) {
                            ...
                            if ($fractions_sold > 0) {
                                ...
                                $remaining_fractions = ($opened_units * $itemStock->dispensedQuantity) - abs($fractions_sold - $fractions_now);
                                // $remaining_fractions = (($opened_units * $itemStock->dispensedQuantity) - ($fractions_sold - $fractions_now))% $itemStock->dispensedQuantity;

                                // dd($remaining_fractions);
                            }
                ...
                Object.values(groupedDetails).forEach(group => {
                    ...
                    // alert(product.stock_details.full_units)
                    // console.log(product);
                    ...
                    details.forEach(detail => {
                        if (detail.dispensed === 'Fraccionado') {
                            ...
                            // Calcular stock a restaurar para validación (fracciones convertidas a unidades)
                            quantityRestored += parseFloat(detail.quantity) / (stock.dispensedQuantity || 1);
                        } else {
                            // Es venta por unidad (Entero)
                            $(`#input-quantity-unit-${rowId}`).val(detail.quantity);
                            $(`#input-price-unit-${rowId}`).val(detail.price);
                            // Usamos el ID del detalle principal para el input hidden (aunque al guardar recrearemos todo)
        ...
        function getSubtotal(id) {
            ...
            // --- INICIO: Lógica de validación de stock mejorada ---
            let total_stock_in_fractions;
            if (product.stock_details) {
                // Usar el stock real calculado si existe (unidades completas + fracciones restantes)
                ...
                // Fallback a la lógica original si no hay stock_details
            ...
            if (requested_fractions > total_stock_in_fractions) {
                ...
                // Reajustar la cantidad al máximo posible
                ...
                // Recalcular cantidades después del ajuste
                             ...
                             <div style="flex-grow: 1; line-height: 1.4;">
                                 ...
                                 <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 8px;">
                                     <div style="font-size: 16px; font-weight: bold; color: #2c3e50; padding-right: 10px;">
                                         <i class="fa-solid fa-pills" style="color: #3498db;"></i> ${option.item.name}
                                     </div>
